...
/**
 * Unit test for TransferController {@link com.lgtm.easymoney.controllers.TransferController}
 * */
...
public class TransferControllerTest {
    ...
    private Long FROM_UID = 1L;
    private Long TO_UID = 2L;
    private BigDecimal AMOUNT = BigDecimal.valueOf(30.0);
    private String DESCRIPTION = "this is a test transfer";
    private Date LAST_UPDATE_TIME = new Date(20221020L);

    @Before
    public void setUp() {
        transferReq = new TransferReq();
        transferReq.setFromUid(FROM_UID);
        transferReq.setToUid(TO_UID);
        transferReq.setAmount(AMOUNT);
        transferReq.setCategory(Category.PARTY);
        transferReq.setDescription(DESCRIPTION);

        transactionRsp = new TransactionRsp(FROM_UID, TO_UID, AMOUNT, TransactionStatus.TRANS_COMPLETE,
                DESCRIPTION, Category.PARTY, LAST_UPDATE_TIME);
    ...
    public void transferSuccess() throws Exception {
        ...
        returnedResponse.andExpectAll(
                ...
                jsonPath("$.transfers[0].fromUid").value(FROM_UID),
                jsonPath("$.transfers[0].toUid").value(TO_UID),
    ...
    public void getTransfersSuccess() throws Exception {
        // Arrange
        Mockito.when(transferService.getTransfersByUid(FROM_UID))
                .thenReturn(ResponseEntity.of(Optional.of(transferRsp)));

        // Act
        ResultActions returnedResponse =
                mvc.perform(get("/transfer/{uid}", String.valueOf(FROM_UID)));
